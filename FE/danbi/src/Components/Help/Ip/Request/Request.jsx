...
const Request = () => {
  ...
  const [minute, setMinute] = useState(
    new Date().getMinutes() - (new Date().getMinutes() % 5) + 5
  );
  ...
  const [position, setPosition] = useState({
    coords: { latitude: 36.110336, longitude: 128.4162384 },
  }); //현재 위치에서 찍을 수 있는 위도, 경도
  ...
  const SendRequestEdit = useCallback(async () => {
    ...
    const callback = async (result, status) => {
      if (status === window.kakao.maps.services.Status.OK) {
        ...
        const data = {};
        const startTime = `${year}-${month + 1 < 10 ? '0' + (month + 1) : month + 1}-${
          day < 10 ? '0' + day : day
        ...
        data['end_time'] = `${endTime.getFullYear()}-${
    ...
    <Wrap>
      ...
      <Main $full={false}>
        {tap === 'dest' || tap === 'meet' ? (
          ...
          <>
            <Tap>
              <TapItem $on={tap === 'time'} onClick={() => setTap('time')}>
                시간예약
              </TapItem>
              <TapItem $on={tap === 'setting'} onClick={() => setTap('setting')}>
                상세설정
              </TapItem>
            </Tap>
            <MainWrap $full={false}>
              ...
              {tap === 'setting' ? (
                <Setting
                  content={content}
                  SendRequestEdit={SendRequestEdit}
                  helpPostId={helpPostId}
                  SendRequest={SendRequest}
                  presets={presets}
                  setCautionTitle={setCautionTitle}
...
const TapItem = styled.button`
  border-bottom: 3px solid
    ${(props) => (props.$on ? props.theme.colors.titleColor : 'transparent')};
`;
