...
const Admin = () => {
  ...
  useEffect(()=>{
    ...
    const getMember = async() =>{
      try{
        if(mode==="ALL"){
          const data = await authGet(`/api/v1/admin/member?page=${curPage-1}&size=${pagingCount}&sort=id&direction=${direction}`); 
          if(data){
            console.log(data);
          ...
          const data = await authGet(`/api/v1/admin/member/role?memberRole=${mode}&page=${curPage-1}&size=${pagingCount}&sort=id&direction=${direction}`); 
          if(data){
            setUsers(data.members);
